...
//    hiddenaccRowCnt value 넣기
    ...
    budget.addEventListener("change",getNewBudget2);
...
function makediary(){
        ...
        const diadate = diahead.children[1];
        const diaweather = diahead.children[2];
//        cntdia.setAttribute('name','cnt_diary');
        ...
        const string = 'diacont-'+cnt;
        diacont.setAttribute('name',string);
        diacont.setAttribute('id',string);
...
function getNewBudget(e){
  ...
  let containerNum=e.id.split("-")[1];
  let spending ="amount-"+containerNum;
  
  //budget-1='30,000'
  totalAmount[e.id]=e.value;
  // 그냥 그때그때 총액에서 빼서 잔액보여주면 됨
  let result = e.value.replace(/\,/g,'')-totalAmount[spending];

  e.parentElement.parentNode.children[3].children[0].value=
  result.toLocaleString();
  
  
}
function getNewBudget2(e){
      ...
      const balanceInput = obj.parentElement.parentNode.parentNode.nextElementSibling.children[1].children[3].children[0];
       ...
       for(let i=0;i<Object.keys(totalAmount).length; i++){
           if((Object.keys(totalAmount)[i])==obj.id){
               ...
               // console.log("a의값: "+a);
               if(isNaN(a)){
                   totalAmount[spending]-=a.replaceAll(/\,/g,'');
               }
               else{
                   // console.log("여기로 들어옴")
